...
- **Breed popularity over time (2013–2020)** based on yearly AKC rank (1 = most popular).  
...
renderPlotly({
  ...
  df <- breed_traits %>%
    select(Breed, 
           x_trait = all_of(input$trait_x), 
           y_trait = all_of(input$trait_y))
  
  # Calculate correlation for these two traits
  cor_value <- cor(df$x_trait, df$y_trait, use = "complete.obs")
  
  p <- ggplot(df, aes(x = x_trait, y = y_trait, text = Breed)) +
    geom_jitter(width = 0.1, height = 0.1, alpha = 0.6, color = "#8C5A3C", size = 2) +
    geom_smooth(method = "lm", se = TRUE, color = "#D4A574", fill = "#D4A574", alpha = 0.2) +
    scale_x_continuous(breaks = 1:5) +
...
renderDT({
  ...
  trait_cor_long <- trait_cor %>%
    ...
    filter(Trait1 != Trait2) %>%  # remove self-correlations
