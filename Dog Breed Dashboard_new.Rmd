...
# Order traits by their mean score (low → high) and define a shared color palette
...
# One color per trait using viridis
...
This interactive flexdashboard uses American Kennel Club (AKC) data to explore dog breeds from three main angles:

- **Popularity over time (2013–2020)** using yearly AKC ranks (1 = most popular).  
...
📝 Overview: Preview AKC popularity ranks (2013–2020) and how rank works (1 = most popular).

📊 Summary: See key metrics (total breeds, most/least popular, biggest risers/fallers) plus average trait scores and their distributions.

🏆 Popularity: Compare popularity trends for selected breeds over time and view their images, including on hover.

🔍 Breed Traits Explorer: Inspect a single breed’s trait profile (1–5 scores), with sketch, full-color photo, and trait definitions.

🔗 Trait Correlation: Explore a trait correlation heatmap, top correlated trait pairs, and a scatterplot for any two traits with hoverable breed images.


Overall, this dashboard is designed to help you:
- Compare how popular different breeds have been over time.  
- Understand each breed’s behavioral and care traits in detail.  
- Discover patterns and relationships among traits across all breeds, with images and interactivity to make the data more intuitive and fun to explore.
...
renderPlotly({
  ...
  df <- trait_long %>%
    ...
    dplyr::mutate(
      Trait = factor(Trait, levels = trait_order)  # same global order (low → high)
    ) %>%
    dplyr::arrange(Trait)
...
### 🔍 Select Traits to Explore Correlation {data-height=55}
```{r}
fluidRow(
  column(
    width = 7,
...
### 📉 Scatterplot: Selected Traits {data-height=45}
```{r}
renderPlotly({
