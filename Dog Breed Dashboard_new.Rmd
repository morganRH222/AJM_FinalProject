...
This flexdashboard provides an interactive exploration of dog breed information compiled from the American Kennel Club (AKC). It highlights two major dimensions of breed data—popularity trends and behavioral/care traits—and presents them in a format designed to help users compare breeds, understand key characteristics, and interpret trait scoring.

What this dashboard includes:

- **Breed popularity over time (2013–2020)**
Visualizes each breed’s AKC popularity ranking over time (with Rank 1 representing the most popular breed). Users can track long-term trends, spot sudden shifts in ranking, and compare multiple breeds side by side.

- **Behavior and care traits**
Displays breed scores (1–5 scale) on traits such as affection level, energy level, trainability, shedding, barking, and more. These trait profiles help illustrate what living with each breed is typically like.

- **Trait definitions**
Provides clear explanations of what each numerical trait score represents, helping users interpret whether a rating of “2” or “5” corresponds to a low, moderate, or high expression of that trait.

How to use the dashboard:

- Navigate across tabs to explore popularity charts, trait radar plots, and reference definitions.

- Interact with the visualizations to compare breeds, filter by specific years, or examine individual trait profiles.

- Use the definitions tab whenever you need clarity on what a trait score means in real-world behavior or care expectations.
...
renderPlotly({
  trait_avg <- trait_long %>%
    group_by(Trait) %>%
    summarise(
      avg_trait_score = mean(Score, na.rm = TRUE),
      .groups = "drop") %>%
    arrange(avg_trait_score) %>%
    mutate(Trait = factor(Trait, levels = Trait))
  
  p <- ggplot(trait_avg, aes(x = avg_trait_score, y = Trait, fill = avg_trait_score)) +
    geom_col() +
    scale_fill_viridis_c(option = "H") +
    scale_x_continuous(limits = c(0, 5), breaks = 0:5) +
    labs(
      x = "Average Trait Score (0–5)",
      y = "",
  ...
  ggplotly(p, tooltip = c("Trait", "avg_trait_score"))})
```

Column {data-width=37.5}
-----------------------------------------------------------------------

### 📊 Distribution of Trait Scores
```{r}
renderPlotly({
...
### 🔍 Select Traits to Explore Correlation
```{r}
fluidRow(
  column(
    width = 6,
