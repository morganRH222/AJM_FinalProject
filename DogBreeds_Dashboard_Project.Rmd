...
# To Read Dataset Directly from GitHUb
...
# To Wrangle the Data for Analyses
...
## To Get Most Recent Year Rank as "Current" Popularity
...
## To Join Traits and latest rank into one dataframe
...
# To create a "family" and "variant" column for similar breed
This is an option on our dashboard to allow user to compare breeds from the same family (e.g. Golden Retriever and Labrador Retriever)

```{r}

dog_data <- dog_data %>%
  mutate(
    # Family: first word before the first space
    Breed_family  = str_extract(Breed, "^[^ ]+"),
    
    # Variant: text inside parentheses, e.g. (Labrador) -> "Labrador"
    Breed_variant = str_extract(Breed, "(?<=\\().*(?=\\))")
  )

dog_data %>%
  select(Breed, Breed_family, Breed_variant) %>%
  head()

```


## To Create a Vector of Breeds for Dropdown Menu
...
## To Create a Vector of Trait Variables 
...
## To Create a named vector for Select Input Choices
...
```
## Top 10 breeds by 2020 rank 

```{r}
#Creating object with just top 10 breeds for future graphing 
top10_latest <- dog_data %>%
  filter(!is.na(latest_rank)) %>%
  arrange(latest_rank) %>%
  slice_head(n=10)
...
# **About the Dashboard**
This dashboard explores dog breed characteristics and popularity using data from the **TidyTuesday (2022-02-01) Dog Breeds dataset**.  

It includes three main components:

- **Popularity over time:** Choose a breed to see how its AKC ranking has changed from year to year.
- **Trait vs. Popularity:** Select a trait (e.g., Trainability, Shedding Level) and explore how it relates to breed popularity.
- **Interactive Data Table:** Browse and filter all breeds and their characteristics.

**How to use the dashboard:**
- Use the **breed dropdown** to select a breed for the time-trend plot.
- Use the **trait dropdown** to choose which trait to compare against AKC rank.
- Use the interactive table filters to compare breeds.
